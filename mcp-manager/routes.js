...
// Get tools list - fetch from the actual MCP server
const MCP_SERVER_PATH = process.env.MCP_SERVER_PATH || path.join("F:/MCP Database/local-mcp-server", "server.js");
...
router.get('/tools', async (req, res) => {
    ...
    try {
        ...
        const toolsMap = {
            'local-mcp-server': await queryMcpServer()
        };
...
router.post('/save-configs', async (req, res) => {
    ...
    try {
        ...
        await fs.mkdir(path.dirname(CURSOR_CONFIG_PATH), { recursive: true });
